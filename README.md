...
- 创建房间时可在侧边栏“对局时间”选择：**不限时 / 5 分钟 / 10 分钟 / 15 分钟**（默认 **10 分钟**）。
- 黑方与白方各有**独立的倒计时**，当前轮到谁走棋，谁的时间就在走。
- 落子后自动**切换**给对方计时。
- 计时由**主机统一权威管理**：开局把初始时间同步给客户端，客户端仅在本地做显示刷新，不做判负。
- **超时判负**：主机检测到当前回合方时间耗尽即判其负，并通过 `GAME_OVER (reason=TIMEOUT)` 同步到双方。
- **新一局自动重置**双方时间；**断线 / 终局立即停止计时**。

### 联机聊天

- 对局进行中，右侧“对局聊天”面板可**实时聊天**。
- **Enter 或点击“发送”**发送消息。
- 每条消息带**发送者**（我 / 对方）与**时间**。
- 消息长度上限 **200 字**，支持**中文与 Emoji**。
- 复用现有 TCP 连接（`CHAT_MESSAGE` 协议），**不会另外建立第二条聊天连接**；主机校验并拒绝空消息 / 超长消息。
- 聊天**不影响棋盘与计时**；对局结束 / 断线后聊天输入自动禁用。
