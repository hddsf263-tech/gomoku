...
- `NetProtocol`：协议层，定义消息类型（`JOIN` / `WELCOME` / `STATE` / `MOVE` / `REQ_MOVE` / `REJECT` / `GAME_OVER` / `NEW_GAME` / `REMATCH` / `BYE` / `PING` / `PONG`），JSON 换行分隔帧（天然解决 TCP 半包/粘包）。
- `NetLink`：传输层，封装 TCP 监听/连接、帧化与事件信号，不接触棋局核心。
- `OnlineSession`：会话状态机，Host 权威裁判、回合校验、状态广播、胜负判定、重赛与断线处理；事件驱动，不阻塞 UI。

### 创建主机

1. 在主界面选择“网络对战”模式，然后选择“创建主机”（Host）。
2. 设置端口（默认 `12345`），点击“开始”。
3. 程序会显示本机局域网 IP 与监听端口，等待对手加入。
4. 主机扮演黑方并先手。

### 加入主机

1. 在另一台电脑上选择“网络对战”模式，然后选择“加入主机”（Client）。
2. 输入主机的局域网 IP 与端口（默认 `12345`）。
3. 点击“连接”，等待主机确认；连接成功后由主机分配颜色（客户端为白方，后手）。
